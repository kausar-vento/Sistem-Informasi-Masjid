@push('titles')
    <title>Detail Transaksi Penggalangan Dana</title>
@endpush
@extends('admin.component.header')
@section('header-admin')
<div class="card">
    <h5 class="card-header">Detail Transaksi <b>{{$getFR->nama_kegiatan}}</b></h5>
</div>
<div class="card mt-3 col-md-8 mx-auto">
    <div class="card-body">
        <div class="row">
            <div class="col-md-5">
                <img src="{{asset('storage/'.$getFR->thumbnail_donasi)}}" alt=""
                ...
                <ul class="list-group list-group-flush mt-4">
                    ...
                    <li class="list-group-item">
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                @php
                                $getAsli = $getTotalTransaksiFR - ($getTransaksiFR->sum('infaq_masjid'))
                                @endphp
                                @harga($getAsli)
                                <i class="bx bx-chevron-right bx-sm align-middle float-end"></i>
                            </div>
                    ...
                    <li class="list-group-item">
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                @php
                                $getInfaq = $getTransaksiFR->sum('infaq_masjid')
                                @endphp
                                @harga($getInfaq)
                                <i class="bx bx-chevron-right bx-sm align-middle float-end"></i>
                            </div>
                    ...
                    <li class="list-group-item">
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                @php
                                $totalUser = $getTransaksiFR->count();
                                @endphp
                                Jumlah: {{$totalUser}}
                                <a class="float-end" href="{{route('laporanPesertaFR', $getFR->nama_kegiatan)}}">
                ...
                <form id="formAktivitas" action="{{ route('prosesNeraca') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col mb-3">
                            ...
                            <input type="text" id="nominal" name="nominal" class="form-control @error('nominal') is-invalid @enderror" placeholder="Saldo Saat ini @harga($getTotalTransaksiFR)" />
                            @error('nominal')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="jenis_transaksi" value="fund_raising" class="btn btn-primary" id="saveChangesButton">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#nominal').on('input', function () {
            let totalHarga = {{$getTotalTransaksiFR}};
